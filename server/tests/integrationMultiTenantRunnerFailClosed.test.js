...
const RUNNER_PATH = path.join(
  ...
  "tests",
  "integration",
  "osMultiTenantIsolation.integration.js"
);
...
function runRejectedTarget({
  ...
  const tempDir = fs.mkdtempSync(
    path.join(
      os.tmpdir(),
      "os-saas-multitenant-failclosed-"
    )
  );
...
test(
  "runner multi-tenant rejeita banco local normal antes de carregar db.js",
  () => {
...
test(
  "runner multi-tenant rejeita host remoto antes de carregar db.js",
  () => {
    const result = runRejectedTarget({
